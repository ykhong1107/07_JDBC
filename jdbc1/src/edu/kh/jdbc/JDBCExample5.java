...
public class JDBCExample5 {
	...
	public static void main(String[] args) {
		 ...
		 *    
		 * 장점 1 : SQL 작성이 간단해짐
		 * 장점 2 : ?에 값 대입 시 자료형에 맞는형태의 
		 * 			리터럴 표기법으로 대입됨!
		 * 			ex) String 대입 -> '값'  (자동으로 ''추가)
		 * 			ex) int 대입 ->	    값   
		 * 장점 3 : 성능, 속도에서 우위를 가지고있음
		 * 
		 * ** PreparedStatement는 Statement의 자식 **
		 */
		PreparedStatement pstmt = null;
		
		// SELECT가 아니기 때문에 ResultSet 필요없다!
		
		try {
			
			/* 2. DriverManager를 이용하여 Connection 객체생성 */
			Class.forName("oracle.jdbc.driver.OracleDriver");
			
			String url = "jdbc:oracle:thin:@localhost:1521:XE";
			String userName = "KH_HYK";
			...
			if(result > 0) { // INSERT 성공 시
				...
				// INSERT 값이 다 들어있어서 실패는 할 수 가없지만
				// 그래도 이렇게 적어줘야한다 나중을 위해.
